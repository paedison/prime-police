...
class Problem(TimeRemarkBase):
    ...
    open_users = models.ManyToManyField(
        User, related_name='prime_opened_problems', through='ProblemOpen')
    like_users = models.ManyToManyField(
        User, related_name='prime_liked_problems', through='ProblemLike')
    rate_users = models.ManyToManyField(
        User, related_name='prime_rated_problems', through='ProblemRate')
    solve_users = models.ManyToManyField(
        User, related_name='prime_solved_problems', through='ProblemSolve')
    memo_users = models.ManyToManyField(
        User, related_name='prime_memoed_problems', through='ProblemMemo')
    comment_users = models.ManyToManyField(
        User, related_name='prime_commented_problems', through='ProblemComment')
    collections = models.ManyToManyField(
        'ProblemCollect', related_name='collected_problems', through='ProblemCollectedItem')

    class Meta:
        verbose_name = verbose_name_plural = "모의고사 문제"
        unique_together = ['year', 'exam', 'subject', 'number']
        ordering = ['-year', 'id']

    def __str__(self):
        return f'[Prime]Problem:{self.reference}(id:{self.id})'
    ...
    def get_absolute_url(self): return reverse('prime:problem-detail', args=[self.id])

    def get_like_url(self): return reverse('prime:like-problem', args=[self.id])

    def get_rate_url(self): return reverse('prime:rate-problem', args=[self.id])

    def get_solve_url(self): return reverse('prime:solve-problem', args=[self.id])

    def get_tag_add_url(self): return reverse('prime:tag-problem-create', args=[self.id])

    def get_tag_remove_url(self): return reverse('prime:tag-problem-remove', args=[self.id])
...
class ProblemOpen(TimeRemarkBase):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='prime_problem_open_set')
    ip_address = models.TextField(blank=True, null=True)

    class Meta:
        verbose_name = verbose_name_plural = "모의고사 문제 확인 기록"
        db_table = 'a_prime_problem_open'

    def __str__(self): return f'[Prime]ProblemOpen:{self.problem.reference}-{self.user.username}'
...
class ProblemLike(TimeRemarkBase):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='prime_problem_like_set')
    is_liked = models.BooleanField(default=True)

    class Meta:
        verbose_name = verbose_name_plural = "모의고사 문제 즐겨찾기"
        unique_together = ['problem', 'user']
        db_table = 'a_prime_problem_like'

    def __str__(self): return f'[Prime]ProblemLike:{self.problem.reference}-{self.user.username}'
...
class ProblemRate(TimeRemarkBase):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='prime_problem_rate_set')
    rating = models.IntegerField(choices=rating_choices)

    class Meta:
        verbose_name = verbose_name_plural = "모의고사 문제 난이도"
        unique_together = ['problem', 'user']
        db_table = 'a_prime_problem_rate'

    def __str__(self):
        return (f'[Prime]ProblemRate:{self.problem.reference}-{self.user.username}'
...
class ProblemSolve(TimeRemarkBase):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='prime_problem_solve_set')
    answer = models.IntegerField(choices=answer_choices)
    is_correct = models.BooleanField()

    class Meta:
        verbose_name = verbose_name_plural = "모의고사 문제 정답확인"
        unique_together = ['problem', 'user']
        db_table = 'a_prime_problem_solve'

    def __str__(self):
        result = 'O' if self.is_correct else 'X'
        return (f'[Prime]ProblemSolve:{self.problem.reference}-{self.user.username}'
...
class ProblemMemo(TimeRemarkBase):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='prime_problem_memo_set')
    memo = RichTextField(config_name='minimal')

    class Meta:
        verbose_name = verbose_name_plural = "모의고사 문제 메모"
        unique_together = ['problem', 'user']
        db_table = 'a_prime_problem_memo'

    def __str__(self):
        return f'[Prime]ProblemMemo:{self.problem.reference}-{self.user.username}'
...
class ProblemComment(TimeRemarkBase):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='prime_problem_comment_set')
    ...
    class Meta:
        verbose_name = verbose_name_plural = "모의고사 문제 코멘트"
        unique_together = ['problem', 'user']
        db_table = 'a_prime_problem_comment'

    def __str__(self):
        return f'[Prime]ProblemComment:{self.problem.reference}-{self.user.username}'
...
class ProblemTag(TagBase):
    ...
    class Meta:
        verbose_name = "모의고사 문제 태그"
        verbose_name_plural = "모의고사 문제 태그"
        db_table = 'a_prime_problem_tag'

    def __str__(self):
        return f'[Prime]ProblemTag:{self.name}'
...
class ProblemTaggedItem(TimeRemarkBase, TaggedItemBase):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='prime_problem_tagged_items')
    active = models.BooleanField(default=True)

    class Meta:
        verbose_name = verbose_name_plural = "태그된 모의고사 문제"
        unique_together = ('tag', 'content_object', 'user')
        db_table = 'a_prime_problem_tagged_item'


class ProblemCollect(TimeRemarkBase):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='prime_problem_collect_set')
    title = models.CharField(max_length=20)
    order = models.IntegerField()

    class Meta:
        verbose_name = verbose_name_plural = "모의고사 문제 컬렉션"
        unique_together = ["user", "title"]
        db_table = 'a_prime_problem_collect'

    def __str__(self):
        return (f'[Prime]ProblemCollect:{self.title}-{self.user.username}'
...
class ProblemCollectedItem(TimeRemarkBase):
    ...
    class Meta:
        verbose_name = verbose_name_plural = "컬렉션에 추가된 모의고사 문제"
        unique_together = ['collect', 'problem']
        db_table = 'a_prime_problem_collected_item'

    def __str__(self):
        return (f'[Prime]ProblemCollectedItem:{self.problem.reference}'
