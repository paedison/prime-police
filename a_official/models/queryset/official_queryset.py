...
class ExamQuerySet(models.QuerySet):
    def predict_exam_active(self):
        return self.filter(predict_exam__is_active=True).select_related('predict_exam').order_by('-id')
...
class ProblemQuerySet(models.QuerySet):
    def base_list(self, problem):
        return self.filter(exam=problem.exam, subject=problem.subject)
    ...
    def filtered_problem_by_exam(self, exam):
        return (
            self.select_related('exam').filter(exam=exam)
...
class ProblemCollectionQuerySet(models.QuerySet):
    ...
    def user_collection_for_modal(self, user, pk):
        item_model = apps.get_model('a_official', 'ProblemCollectionItem')
        collection_ids = (
